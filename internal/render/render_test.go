...
func TestBuildAggregatesProvidersInStableOrder(t *testing.T) {
	now := time.Now()
	output := Build([]model.Snapshot{
		{Provider: "codex", FetchedAt: now, Windows: []model.Window{{Label: "weekly", UsedPct: 42}}},
		{Provider: "claude", FetchedAt: now, Windows: []model.Window{{Label: "5h", UsedPct: 86}}},
	}, View{}, now)
...
func TestNavigateProviderUsesAggregateBoundaries(t *testing.T) {
	...
	snapshots := []model.Snapshot{
		{Provider: "codex", FetchedAt: now, Windows: []model.Window{{Label: "weekly", UsedPct: 42}}},
		{Provider: "claude", Err: model.NewProviderError(model.ErrorAuth, errors.New("expired"))},
	}

	view := View{WindowIndex: 4}
...
func TestNavigateProviderTogglesSingleProvider(t *testing.T) {
	snapshots := []model.Snapshot{{Provider: "codex", Windows: []model.Window{{Label: "weekly", UsedPct: 42}}}}
...
func TestCycleWindowPreservesAutomaticIndexAndWraps(t *testing.T) {
	...
	now := time.Now()
	snapshots := []model.Snapshot{{Provider: "codex", Windows: []model.Window{
		{Label: "weekly", UsedPct: 20},
		{Label: "5h", UsedPct: 80},
	}}}
	output := Build(snapshots, View{WindowIndex: view.WindowIndex}, now)
...
func TestBuildPinnedProviderIsolatesUnrelatedErrors(t *testing.T) {
	...
	output := Build([]model.Snapshot{
		{Provider: "claude", FetchedAt: now, Windows: []model.Window{{Label: "5h", UsedPct: 86}}},
		{Provider: "codex", Err: model.NewProviderError(model.ErrorAuth, errors.New("expired"))},
	}, View{PinnedProvider: "claude"}, now)
...
func TestTooltipRendersAllProvidersAndWindows(t *testing.T) {
	...
	output := Build([]model.Snapshot{
		{Provider: "codex", Windows: []model.Window{
		...
		{Provider: "claude", Windows: []model.Window{
...
func TestTooltipUsesResetFallbacks(t *testing.T) {
	now := time.Date(2026, time.July, 13, 8, 0, 0, 0, time.UTC)
	output := Build([]model.Snapshot{{Provider: "codex", Windows: []model.Window{
...
func TestJSONMaintainsWaybarContract(t *testing.T) {
	now := time.Now()
	data, err := JSON([]model.Snapshot{{Provider: "codex", FetchedAt: now, Windows: []model.Window{{Label: "weekly", UsedPct: 12}}}}, View{}, now)
	if err != nil {
		t.Fatal(err)
	}
