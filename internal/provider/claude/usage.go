...
func ParseUsage(reader io.Reader, now time.Time) (model.Snapshot, error) {
	...
	if len(data) > maxUsageBody {
		return model.Snapshot{}, errors.New("claude usage response is too large")
	}

	var root map[string]json.RawMessage
	if err := json.Unmarshal(data, &root); err != nil {
		return model.Snapshot{}, fmt.Errorf("decode claude usage response: %w", err)
	}
	...
	for _, candidate := range []struct {
		...
		windows = append(windows, model.Window{Label: candidate.label, UsedPct: clampPercentage(used), ResetsAt: resetsAt, WindowMinutes: candidate.windowMinutes})
	}
	...
	return model.Snapshot{Windows: windows, FetchedAt: now}, nil
}
