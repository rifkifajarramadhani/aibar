...
func (p *Provider) Fetch(ctx context.Context) (model.Snapshot, error) {
	now := p.config.Now()
	credentials, err := LoadCredentials(p.config.CredentialsPath, now)
	if err != nil {
		return model.Snapshot{Provider: p.Name(), Source: model.SourceNetwork, FetchedAt: now, Err: err}, err
	}
	...
	if err != nil {
		err = model.NewProviderError(model.ErrorNetwork, errors.New("create claude usage request"))
		return model.Snapshot{Provider: p.Name(), Source: model.SourceNetwork, FetchedAt: now, Err: err}, err
	}
	...
	if err != nil {
		...
		classified := model.NewProviderError(model.ErrorNetwork, &httpFailure{err: errors.New(message), cause: err})
		return model.Snapshot{Provider: p.Name(), Source: model.SourceNetwork, FetchedAt: now, Err: classified}, classified
	}

	defer func() { _ = response.Body.Close() }()

	if response.StatusCode == http.StatusUnauthorized || response.StatusCode == http.StatusForbidden {
		err = model.NewProviderError(model.ErrorAuth, &httpFailure{status: response.StatusCode, err: errors.New("claude usage request was unauthorized")})
		return model.Snapshot{Provider: p.Name(), Source: model.SourceNetwork, FetchedAt: now, Err: err}, err
	}

	if response.StatusCode == http.StatusTooManyRequests {
		retryAfter := parseRetryAfter(response.Header.Get("Retry-After"), now)
		err = model.NewProviderError(model.ErrorRateLimit, &httpFailure{status: response.StatusCode, retryAfter: retryAfter, err: errors.New("claude usage request was rate limited")})
		return model.Snapshot{Provider: p.Name(), Source: model.SourceNetwork, FetchedAt: now, Err: err}, err
	}

	if response.StatusCode < http.StatusOK || response.StatusCode >= http.StatusMultipleChoices {
		kind := model.ErrorNetwork
		if response.StatusCode >= http.StatusBadRequest && response.StatusCode < http.StatusInternalServerError {
			kind = model.ErrorParse
		}

		err = model.NewProviderError(kind, &httpFailure{status: response.StatusCode, err: fmt.Errorf("claude usage request returned HTTP %d", response.StatusCode)})
		return model.Snapshot{Provider: p.Name(), Source: model.SourceNetwork, FetchedAt: now, Err: err}, err
	}

	snapshot, err := ParseUsage(response.Body, now)
	if err != nil {
		err = model.NewProviderError(model.ErrorParse, err)
		return model.Snapshot{Provider: p.Name(), Source: model.SourceNetwork, FetchedAt: now, Err: err}, err
	}

	snapshot.Provider = p.Name()
	snapshot.Source = model.SourceNetwork

	return snapshot, nil
}

func (p *Provider) Refresh(context.Context, chan<- model.Snapshot) {
	select {
...
func emit(ctx context.Context, out chan<- model.Snapshot, snapshot model.Snapshot) {
	select {
	case out <- snapshot:
	case <-ctx.Done():
	}
