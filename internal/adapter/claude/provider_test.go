...
func TestFetchParsesUsageAndSendsOAuthHeaders(t *testing.T) {
	...
	if snapshot.Provider != "claude" || snapshot.Source != usage.SourceNetwork || len(snapshot.Windows) != 1 || snapshot.Windows[0].UsedPct != 25 {
...
func TestFetchClassifiesAuthAndRateLimitErrorsWithoutResponseBody(t *testing.T) {
	tests := []struct {
		...
		status     int
		retryAfter string
		kind       usage.ErrorKind
	}{
		{name: "auth", status: http.StatusUnauthorized, kind: usage.ErrorAuth},
		{name: "forbidden", status: http.StatusForbidden, kind: usage.ErrorAuth},
		{name: "rate limit", status: http.StatusTooManyRequests, retryAfter: "600", kind: usage.ErrorRateLimit},
	}
...
func TestFetchClassifiesServerAndParseErrors(t *testing.T) {
	tests := []struct {
		...
		{name: "server", status: http.StatusBadGateway, body: "bad gateway", kind: usage.ErrorNetwork},
		{name: "malformed", status: http.StatusOK, body: "{", kind: usage.ErrorParse},
		{name: "missing windows", status: http.StatusOK, body: `{}`, kind: usage.ErrorParse},
	}
...
func TestNextDelayEnforcesMinimumAndBacksOffTransientFailures(t *testing.T) {
	...
	transient := usage.NewProviderError(usage.ErrorNetwork, &httpFailure{status: http.StatusBadGateway, err: errors.New("temporary failure")})
	...
	rateLimited := usage.NewProviderError(usage.ErrorRateLimit, &httpFailure{retryAfter: 10 * time.Minute, err: errors.New("rate limited")})
...
func TestFetchHonorsCanceledContext(t *testing.T) {
	...
	if usage.ErrorKindOf(err) != usage.ErrorNetwork || requests.Load() != 0 {
...
func TestFetchHonorsHTTPTimeout(t *testing.T) {
	...
	if usage.ErrorKindOf(err) != usage.ErrorNetwork {
