...
func ParseFile(path string, now time.Time) (usage.Snapshot, error) {
	...
	snapshot.Source = usage.SourceLocal
	snapshot.FetchedAt = now

	return snapshot, nil
}

func Parse(reader io.Reader, now time.Time) (usage.Snapshot, error) {
	...
	return usage.Snapshot{Windows: windowsFrom(latest.RateLimits, now)}, nil
}
...
func Scan(root string, now time.Time) (usage.Snapshot, error) {
	path, err := FindNewest(root)
	if err != nil {
		return usage.Snapshot{Provider: "codex", Source: usage.SourceLocal, FetchedAt: now, Err: err}, err
	}
...
func windowsFrom(limits rateLimits, now time.Time) []usage.Window {
	...
	for _, limit := range []*rateLimit{limits.Primary, limits.Secondary} {
		...
		windows = append(windows, usage.Window{Label: label, UsedPct: used, ResetsAt: resetsAt, WindowMinutes: limit.WindowMinutes})
	}
...
func dedupeWindows(windows []usage.Window) []usage.Window {
