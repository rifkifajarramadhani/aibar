...
var (
	_ usage.Provider    = (*Watcher)(nil)
	_ usage.Refreshable = (*Watcher)(nil)
)
...
func (w *Watcher) Fetch(context.Context) (usage.Snapshot, error) {
	err := errors.New("codex uses its local rollout source")
	return usage.Snapshot{Provider: w.Name(), Source: usage.SourceLocal, Err: err}, err
}

func (w *Watcher) Watch(ctx context.Context, out chan<- usage.Snapshot) error {
...
func (w *Watcher) Rescan(ctx context.Context, out chan<- usage.Snapshot) {
	w.emitScan(ctx, out)
}

func (w *Watcher) Refresh(ctx context.Context, out chan<- usage.Snapshot) {
	w.Rescan(ctx, out)
}

func (w *Watcher) emitScan(ctx context.Context, out chan<- usage.Snapshot) {
	now := w.Now()
