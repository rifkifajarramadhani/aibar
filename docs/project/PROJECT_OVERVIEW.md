...
aibar follows domain-first Clean Architecture; see `docs/ARCHITECTURE.md` for the
package map, dependency rules, and layering enforcement. The core domain lives in
`internal/usage` (entities, `Store` merge policy, ports) and the application loop
in `internal/daemon`; providers, presentation, control, and persistence are
adapters under `internal/adapter/`, wired together by `internal/bootstrap`.

```text
codex fsnotify watcher ─┐
                        │
claude network fetch ───├─> snapshot channel ─> usage.Store ─> waybar renderer ─> stdout
                        │                          │                    │
(future providers) ─────┘                          └─> statefile         └─> Waybar
                                                       (state.json)
...
The core domain contracts live in `internal/usage`. The provider abstraction is
intentionally shared by local and network-backed providers:
